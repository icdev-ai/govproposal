...
Logs all AI model interactions with SHA-256 prompt/response hashing.
...
class AITelemetryLogger:
    """Append-only AI interaction telemetry."""
    ...
    def hash_text(text: str) -> str:
        """SHA-256 hash for prompt/response content."""
        if not text:
            return ""
    ...
    def log_ai_interaction(
        ...
        """Log an AI interaction to the ai_telemetry table (append-only)."""
...
def main():
    ...
    if args.summary:
        result = logger.get_usage_summary(
            project_id=args.project_id, user_id=args.user_id, hours=args.window,
        )
    elif args.anomalies:
